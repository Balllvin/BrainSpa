...
export function TelegramPage() {
  ...
  async function addBot(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await createTelegramBot({
      ...
      model_key: String(form.get("model_key") || "starter_model"),
      enabled: true,
    });
    setSaving(false);
    if (!response.ok) {
    ...
    <section className="panel stack settings-section">
      ...
      <form className="settings-fields settings-fields-bordered" onSubmit={addBot}>
        ...
        <label className="field">
          ...
          <small className="field-hint">Use model or notification names such as starter, notify-evidence, or tune-alerts.</small>
        </label>
        <label className="field">
          <span>Bot token</span>
        ...
        <label className="field">
          <span>Model</span>
          <select name="model_key" defaultValue="starter_model">
            {models.map((m) => (
