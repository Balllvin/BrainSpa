...
def _seed_approved_evidence(tmp_path, claim_count: int = 2) -> None:
    ...
    manifest = {
        ...
        "behavior_focus": "Blunt believer persona",
...
def test_generate_blocked_without_approved_evidence(monkeypatch, tmp_path):
    ...
    response = client.post(
        "/api/datasets/believer_seed/generate",
...
def test_grounded_rows_include_claim_ids(monkeypatch, tmp_path):
    ...
    rows = [json.loads(line) for line in train_jsonl_path("believer_seed").read_text().splitlines() if line.strip()]
...
def test_preview_only_does_not_write_files(monkeypatch, tmp_path):
    ...
    assert not train_jsonl_path("believer_seed").exists()
...
def test_generate_and_rows_crud(monkeypatch, tmp_path):
    ...
    rows = client.get("/api/datasets/believer_seed/rows?limit=5")
    ...
    patched = client.patch(
        f"/api/datasets/believer_seed/rows/{row_id}",
        json={"assistant_answer": "Pray plainly and obey the next step."},
    )
    assert patched.status_code == 200

    deleted = client.delete(f"/api/datasets/believer_seed/rows/{row_id}")
    assert deleted.status_code == 200
    after = client.get("/api/datasets/believer_seed/rows")
...
def test_create_manual_row(monkeypatch, tmp_path):
    ...
    created = client.post(
        "/api/datasets/believer_seed/rows",
        json={
            "scenario_key": "witness",
            "user_prompt": "Someone said faith is only coping.",
            "assistant_answer": "Faith is trust in Christ, not a trick to feel better.",
            "failure_labels": ["generic_slop"],
        },
    )
    assert created.status_code == 200
    assert created.json()["scenario_key"] == "witness"

    listed = client.get("/api/datasets/believer_seed/rows")
    assert listed.json()["total"] == 1


def test_witness_heavy_pack_skews_mix(monkeypatch, tmp_path):
    monkeypatch.setenv("BRAIN_SPA_HOME", str(tmp_path))
    _seed_approved_evidence(tmp_path)
    client = TestClient(create_app())
    ...
    response = client.post(
        "/api/datasets/believer_seed/generate",
        json={"pack": "witness-heavy"},
    )
    assert response.status_code == 200
    mix = response.json()["scenario_mix"]
    assert mix.get("witness", 0) >= 4
...
def test_import_test_feedback_is_idempotent(monkeypatch, tmp_path):
    ...
    _seed_approved_evidence(tmp_path)
    client = TestClient(create_app())
    client.post("/api/datasets/believer_seed/generate", json={"example_count": 8})
    ...
    record = {
        ...
        "answer": "Stay positive and trust the universe.",
        "feedback": "Answer the question directly with Christ-centered conviction.",
        "harness_message_id": 42,
        "feedback_message_id": 43,
    }
    feedback_file.write_text(json.dumps(record) + "\n", encoding="utf-8")

    first = client.post("/api/datasets/believer_seed/import-test-feedback")
    assert first.status_code == 200
    assert first.json()["imported_count"] == 1

    second = client.post("/api/datasets/believer_seed/import-test-feedback")
