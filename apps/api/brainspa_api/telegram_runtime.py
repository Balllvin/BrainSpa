...
from .workflows import starter_runtime_reply, chipmunk_reply, looks_like_loop_request, run_environment_eval
...
def _handle_message(bot: dict[str, Any], message: dict[str, Any], client: TelegramClient) -> dict[str, int]:
    ...
    answer, routed_to, model_name = _build_telegram_reply(bot_name, str(bot.get("model_key") or "starter_model"), text)
        ...
        _record_model_message(
            {
                ...
                "model_key": str(bot.get("model_key") or "starter_model"),
                "model": model_name,
                "routed_to": routed_to,
                "prompt": text,
                "answer": answer,
...
def _build_telegram_reply(bot_name: str, model_key: str, text: str) -> tuple[str, str, str | None]:
    if not _is_chipmunk_bot(bot_name) and model_key and not looks_like_loop_request(text):
        result = starter_runtime_reply(text, model_key)
...
def _save_feedback_if_model_reply(
    ...
    eval_result = run_environment_eval(
        EvalRunRequest(environment_key="chat_starter", prompt=str(record.get("prompt") or ""), answer=str(record.get("answer") or ""))
    )
    log_event(
