...
import { getProjectRunsServer, ServerApiError } from "@/lib/server-api";
...
export default async function ProjectRunsPage({
  ...
  const nextPath = `/projects/${projectId}/runs`;
  ...
  try {
    const data = await getProjectRunsServer(Number(projectId));
    return <ProjectRunsClient initialData={data} />;
  } catch (error) {
